        ...
        <div className='rounded-md mb-3 border p-0'>
            <div className={`p-2 bg-gray-50 flex justify-between ${open ? 'rounded-t-md' : 'rounded-md'} items-center`}>
                <span className='mr-3 w-8/12'>
                    Question {qn?.id + ": "} {qn?.text ? qn?.text.substring(0, 50) : undefined}
                </span>
            ...
            <div className={`${open ? 'p-2' : 'hidden'}`}>
                <div className='mb-3 flex justify-between items-center'>
                    <div className='mr-3 w-8/12 flex-col justify-center items-center'>
                        <textarea disabled={trigger ? false : true} placeholder='type our question here' type="text" value={qn?.text} onChange={(e) => trigger ? updateQuestion({ ...qn, text: e.target.value }) : undefined} name="question" className="mt-1 h-16 max-h-16 block w-full px-3 py-2 bg-white border-2 rounded-md focus:outline-none focus:border-sky-500 hover:border-sky-500"></textarea>
                        <label className="text-red-500">{error} <i></i></label>
                    </div>
                    <div className={`mr-3 flex justify-center ${error ? 'pb-5' : ''} w-2/12`}>
                        <select value={qn?.type} disabled={trigger ? false : true} onChange={(e) => trigger ? updateQuestion({ ...qn, type: e.target.value }) : undefined} name="type" className="w-full p-2 h-16 max-h-16 bg-white border-2 rounded-md focus:outline-none focus:border-sky-500 hover:border-sky-500">
                ...
                <div className=''>
                    {
                        qn?.type === 'structured' ? <label className='border-b-2 border-slate-800 border-dotted' >structured answer</label> : <MultipleChoice hide={remove ? true : false} question={qn} updateQuestion={updateQuestion} />
                    }
                </div>
            </div>
        </div>
