...
function UserModal({ handleSubmit, error, show, processing }) {

    useEffect(() => {
        if (!show) {
            document.getElementById('close').click()
        }
    }, [error, show, processing])
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="relative flex-auto p-4" data-te-modal-body-ref>
                            <div>
                                <label>Name</label>
                                <div>
                                    <input type="text" name="name" className="mt-1 block w-full px-3 py-2 bg-white border rounded-md focus:outline-none focus:border-sky-500" autoFocus />
                                </div>
                            </div>

                            <div className='text-red-500 my-3' id="error">
                                {error}
                            </div>
                        ...
                        <div className="flex flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
                            <button disabled={processing}
                                ...
                                {
                                    processing ? <Spinner text="Processing..." /> : "Add"
                                }
                            </button>
                        </div>
                    </form>
